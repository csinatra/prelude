...
# ── PRELUDE INJECTION (only change vs upstream agents/aide/start.sh, ──
# ── pinned at commit 507f92e) ─────────────────────────────────────────
# If a spec artifact is mounted at /home/spec/spec.md (the per-run spec.md
# produced by harness.runner — Condition B1/B2/C1/C2 output), append it as
# a delimited section after the competition instructions. Absent mount =
# no-op, so this variant degrades to stock aide behavior.
# Header text must stay in sync with harness/injection.py::SPEC_HEADER.
if [ -f /home/spec/spec.md ]; then
  printf "\nADVISOR CONTEXT\n------\n\n" >> ${AGENT_DIR}/full_instructions.txt
  cat /home/spec/spec.md >> ${AGENT_DIR}/full_instructions.txt
fi
# ── END PRELUDE INJECTION ─────────────────────────────────────────────
